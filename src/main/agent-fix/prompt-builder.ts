...
export function buildFixPrompt(annotation: Annotation): string {
  ...
  lines.push('')
  lines.push('Inspecting the live page: the telescope skill already has it open. Prefer:')
  lines.push('  telescope snapshot -i -f <frameId>     # element refs + accessibility tree')
  lines.push('  telescope get styles @<ref>            # computed CSS for an element')
  lines.push('  telescope get text @<ref>              # text content')
  lines.push('  telescope get box @<ref>               # bounding box')
  lines.push(`  telescope eval '<js>'                  # run JS in the page, returns result`)
  lines.push('  telescope screenshot -f <frameId>      # then Read the printed path to view')
  lines.push('Do not use chrome-devtools or any other browser automation tool — telescope')
