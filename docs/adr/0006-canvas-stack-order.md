# ADR 0006 — Canvas stack order and the Notes/Pages sidebar
...
1. **Pure math module + invariant helper.** ✅ Landed. Pure helpers live in `src/shared/entity-order-math.ts` (`bringToFront`, `sendToBack`, `moveForward`, `moveBackward`, `moveBefore(id, anchorId, 'before' | 'after')`, `enforceGroupContiguity(order, groups)`, `appendAtTop`). The runtime wrapper `src/main/runtime/entity-order-state.ts` is now the single source of truth at runtime: it owns the ordered mirror, reconciles entity creation / deletion (new ids appended at the top per §8), and exposes the four mutation primitives. `syncEntityOrder` now writes the runtime mirror to Y.Doc verbatim instead of regenerating from kind buckets; `syncDocToRuntime` mirrors Y.Doc back into the runtime on undo / redo.
...
9. **HTTP API.** ✅ Landed. `src/main/routes/stack-order.ts` exposes `GET /stack-order`, `POST /stack-order/bring-to-front`, `POST /stack-order/send-to-back`, `POST /stack-order/bring-forward`, `POST /stack-order/send-backward`. Each mutation route returns `{ changed, entityOrder }` so smoke tests and agents can drive reorders end-to-end and assert against the runtime mirror.
