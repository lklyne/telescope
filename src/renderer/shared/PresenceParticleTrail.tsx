    ...
    If(age.greaterThan(effectiveLifetime), () => {
      ...
      If(mode.equal(float(PARTICLE_MODE_TRAIL)), () => {
        ...
        .ElseIf(mode.equal(float(PARTICLE_MODE_ORBIT_SPHERE)), () => {
          ...
          // cursor's accumulated rotation angle. Rotation is around the
          // screen's vertical axis, so lat → y offset, (cos, sin) of
          // longitude → (x, z). When a cursor leaves orbit mode the machine
          // triggers a burst, which converts these particles to outward
          // velocity drift via convertToBurstKernel.
          const cIdx = uint(state.y)
          const cursor = cursorPos.element(cIdx)
