...
describe('easeAt', () => {
  ...
  it('custom bezier matches the ease-out preset shape', () => {
    // cubic-bezier(0,0,0.58,1) is a standard ease-out; at t=0.5 y > 0.5
...
describe('deriveControlPoints', () => {
  ...
  it('direction left and right produce opposite-sign perpendicular offsets', () => {
    const paramsLeft = {
      ...DEFAULT_CURSOR_MOTION,
      curveStrength: 0.5,
    ...
    const right = deriveControlPoints(p0, p3, paramsRight, 0)
    expect(Math.sign(left.p1.y)).not.toBe(Math.sign(right.p1.y))
    expect(left.p1.y).toBeCloseTo(-right.p1.y)
  })

  it('alternating flips sign on odd sequence index', () => {
    const params = {
      ...DEFAULT_CURSOR_MOTION,
      curveStrength: 0.5,
    ...
    const odd = deriveControlPoints(p0, p3, params, 1)
    expect(Math.sign(even.p1.y)).not.toBe(Math.sign(odd.p1.y))
  })
...
describe('sampleCursorPath', () => {
  it('endpoints match p0 and p3 regardless of params', () => {
    const p0 = { x: 0, y: 0 }
    const p3 = { x: 100, y: 50 }
    const params = {
      ...DEFAULT_CURSOR_MOTION,
...
describe('normalizeCursorMotion', () => {
  ...
  it('rejects unknown preset name by falling back to default', () => {
    const n = normalizeCursorMotion({ easing: { kind: 'preset', name: 'bogus' } })
    expect(n.easing).toEqual(DEFAULT_CURSOR_MOTION.easing)
  })
})
